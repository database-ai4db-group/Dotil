...
class RandomPolicy:
    def __init__(self, config):
        ...
        # self.flag = self.cf.get("total", "db_flag")  # 判断是哪个数据集
        ...
        self.graph = Graph(self.cf.get("rdb-gdb", "graph_uri"), username='neo4j', password='123456', secure=False,
                           bolt=False)  # neo4j连接
        self.table = self.cf.get("total", "table")
        ...
        self.run_batches = 0  # 记录已经跑的batch数
    ...
    def random_policy(self):
        ...
        for batch_number in range(batch_number):
            ...
            random_choose = random.sample(range(len(former_sql)), end_batch_number - start_batch_number)
            # 将原来的全归零
                ...
                if flag:
                    for p in sub_p_list:
                        if not full_flag and p in self.transfer_record:
                            ...
                            break
                        if int(self.numbers[p]) + self.total_neo4j_number >= self.total_number * self.transfer_percent:
                            full_flag = True
                if full_flag:
                    break
            print(random_choose)
    ...
    def transfer_neo4j_import_no_merge(self, p_list):
        ...
        sql = "SELECT sname, pname, oname FROM " + self.table + " WHERE pname in {};".format(format_p)
        print(sql)
        try:
            cursor.execute(sql)
            results = cursor.fetchall()
...
if __name__ == '__main__':
    ...
    q = RandomPolicy(sys.argv[1].replace("\r", ""))
    q.random_policy()
    q.random_policy()
